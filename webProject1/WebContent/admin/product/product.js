...
function go_detail(tpage, pseq) {
	...
	theForm.action =  "adminProductDetail.do?tpage=" +
	                  tpage+"&pseq="+pseq;
...
function go_mod(tpage, pseq) {
	...
	theForm.action = "adminProductUpdateForm.do?tpage=" + 
		              tpage+"&pseq="+pseq;
	theForm.submit();
}
...
function go_mod_save(tpage, pseq) {
	...
	if (theForm.kind.value == '') {
		...
		if (confirm('수정하시겠습니까?')) {
			...
			theForm.action = "adminProductUpdate.do";
			theForm.submit();
		}
	}
}
...
function go_mod_mov(tpage, pseq) {
	var theForm = document.frm;
	theForm.action = 'adminProductDetail.do?tpage=' + tpage + "&pseq=" + pseq;
	theForm.submit();
}
